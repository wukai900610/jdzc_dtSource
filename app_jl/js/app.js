...
app.dtCharts = function() {
    var prex='http://localhost:3333/credit-data-platform/dynamicMap/';
    var api = {
    ...
    var mapArea = [
        {name: '长春市',selected:true,section:'220100'},
        {name: '吉林市',section:'220200'},
        {name: '四平市',section:'220300'},
        {name: '辽源市',section:'220400'},
        {name: '通化市',section:'220500'},
        {name: '白山市',section:'220600'},
        {name: '松原市',section:'220700'},
        {name: '白城市',section:'220800'},
        {name: '延边朝鲜族自治州',section:'222400'}
    ];
    var mainMapOption = {
        series: [
            {
 ...
 //        setMapArea(mainMapIndex);
	...
	// 	setMainIE(mapArea[mainMapIndex].section);
 ...
 //        setChart9Data(mapArea[mainMapIndex].section);
    ...
    function setMainIE(sectionArea) {
		if(!sectionArea){
            sectionArea = mapArea[0].section
        }

        $.getJSON(api.totalCount,{year:YEAR,section:sectionArea}, function(callbackData){
        ...
        $.getJSON(api.latestHalfYearProductTradeVolume,{year:YEAR,section:sectionArea},function(callbackData){
			...
			$.each(tempObj,function(index,item){
	            ...
	            for(var i=0;i<item.length;i++){
	                temp.push(item[i]['SUM(USD)']/10000)
	                chart2_Option.xAxis[0].data.push(item[i]['MONTH']+'月');
	            }
    ...
    function setChart3Data(sectionArea){
        ...
        $.getJSON(api.latestHalfYearProductTradeVolume,{year:YEAR,section:sectionArea},function(callbackData){
			...
			$.each(tempObj,function(index,item){
			    ...
			    for(var i=0;i<item.length;i++){
			        temp.push(item[i]['SUM(USD)']/10000)
			        chart3_Option.xAxis[0].data.push(item[i]['MONTH']+'月');
			    }
    ...
    function setChart10Data(sectionArea){
        ...
        $.getJSON(api.eiCount,{year:YEAR,section:sectionArea}, function(callbackData){
			...
			//数据排序
			importCount.sort(function(a,b){
				return Date.parse(a.date) - Date.parse(b.date);//时间正序
			});
			exportCount.sort(function(a,b){
				return Date.parse(a.date) - Date.parse(b.date);//时间正序
			});

            $.each(importCount,function(index,item){
                if(item.value == null){
                    item.value = 0;
                }
                dataArr.I.push((item.value/100000000).toFixed(2));
                xAxisData.push(item.date);
            });
            $.each(exportCount,function(index,item){
				if(item.value == null){
                    item.value = 0;
                }
                dataArr.E.push((item.value/100000000).toFixed(2));
            });
